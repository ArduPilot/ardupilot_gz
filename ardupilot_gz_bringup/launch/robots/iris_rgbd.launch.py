...
def generate_robot_launch_actions(context: LaunchContext, *args, **kwargs):
    ...
    sdf_file = os.path.join(
        pkg_project_description, "models", "iris_with_rgbd", "model.sdf"
    )
    ...
    # for sdformat_urdf plugin used by robot_state_publisher
    robot_desc = robot_desc.replace(
        "model://rgbd_cam", "package://ardupilot_gz_description/models/rgbd_cam"
    )

    log = LogInfo(msg="Using iris_with_rgbd (RGB + RGBD camera, iris_rgbd_bridge)")
    config_name = "iris_rgbd_bridge.yaml"
